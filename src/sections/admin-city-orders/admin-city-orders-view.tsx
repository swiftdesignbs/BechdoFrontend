...
import { CityOrdersTable } from './city-orders-table';
import { CityOrdersFilter } from './city-orders-filter';
...
export function AdminCityOrdersView() {
  // ...existing code...
  ...
  const DUMMY_ORDER_TYPES = [
  { id: 'customer', brand_name: 'Customer Orders' },
  { id: 'channel_partner', brand_name: 'Channel Partner Orders' },
];
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      const result = await apiService.getCityOrders(Object.fromEntries(params.entries()));
      if (result.success) {
  ...
  return (
    <Container maxWidth={false}>
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
        <Typography variant="h4">City Orders</Typography>
      </Stack>
      <CityOrdersFilter
        filters={filters}
        onChange={setFilters}
        order_type={DUMMY_ORDER_TYPES}
      />
      <Card>
        <CityOrdersTable
          orders={orders}
